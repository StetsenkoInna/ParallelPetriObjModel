...
public class PetriSim implements Serializable, Runnable {
    ...
    private static long timeInputMeasure=0;
    private static long timeOutputMeasure=0;
...
//        }
    ...
    public void addTimeExternalInput(double t) {
        ...
        try {
            ...
            getCond().signalAll(); // was signal changed by Inna 11.02.2021
        } finally {
            getLock().unlock();
        }
    }
...
//непотрібно перевіряти умову, більше не може бути!! перевірено, що інакше не буває!!! виявлено при моделюванні 
            previousObj.getLock().lock();
            try {
                previousObj.getCondLimitEvents().signalAll(); //was signal 11.02.2021, useful if more than one is prev obj
            } finally {
...
//        }
    ...
    public boolean isStop() {
        // rewritten for parallel by Inna 20.05.2016
            ...
            if (previousObj != null) {
                ...
                try {
                    while (getTimeExternalInput().isEmpty()) {
                        ...
                        this.getCond().await();
//                        System.out.println(this.getName()+" is waiting in run...."+ ((ReentrantLock)getLock()).getHoldCount());
                    }
            ...
            if (getTimeLocal() < limitTime) {    // може бути що декілька подій в буфері в однаковий момент
                //   System.out.println(this.getName() + " will go until " + limitTime + ", have local time " + getTimeLocal());
                goUntil(limitTime);
            } else if (getTimeLocal() == limitTime) { // added 22.02.2021???якщо в буфері 2 однакових то limTime може не просунутись
// тобто first = tloc.....цей фрагмент потребує перевірки...
                if (previousObj != null) {
                    //*****************
                    ...
                    //*****************
                    removeTimeExternalInput(); //видалення зі списку запланованих подій, signal
                    // System.out.println(this.getName() + " will not go until " + limitTime + ", have local time " + getTimeLocal() + ">= time modeling!");
//                return;
                }
            } else {
                System.out.println(this.getName() + " have local time " + getTimeLocal() + " > limit time " + limitTime + ". So return !!!");
                return;
            }
        }
    ...
    public void goUntil(double limit) {
        while (getTimeLocal() < limit) {  //просування часу в межах відведеного інтервалу
            ...
            if (getTimeMin() < limit) {  // будемо продовжувати goUntil, є внутрішні події
                moveTimeLocal(getTimeMin());
                output(); //  signal
                //    System.out.println(this.getName() + " was done output, new value of timeLocal =  " + getTimeLocal());
            } else if (limit >= getTimeMod()) { // усі внутрішні події вичерпані, залишилась зовнішня - на кінці безпечного інтервалу
                moveTimeLocal(getTimeMod());
                if (nextObj != null) {
                    nextObj.addTimeExternalInput(Double.MAX_VALUE); // тобто не очікуємо подій ззовні, signal
                }
            } else if (previousObj != null) {
//                if (getLastTimeExternalInput() < Double.MAX_VALUE) {  //prev obj is not stoped its work,  it seems that this cond is not needed
               /*  removed as a result of modeling....   
                    this.getLock().lock();
                    try {
                        while (getTimeExternalInput().isEmpty()) { // перебудовано 12.01.2021 by Inna
...
//                }
                if (getFirstTimeExternalInput() > getTimeMod()) { // завершити моделювання в об'єкті 
                    if(getFirstTimeExternalInput()!= limit){
                        System.out.println("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
                    }
                    moveTimeLocal(getTimeMod());

                    if (nextObj != null) {
                        nextObj.addTimeExternalInput(Double.MAX_VALUE); // тобто не очікуємо подій ззовні у наступному об'єкті, signal
                    }
                } else*/ {  // виконати зовнішню подію
                    moveTimeLocal(limit);
                    //*****************
                    reinstateActOut(previousObj.getOutT().get(0), previousObj.getListP()[previousObj.getListP().length - 1]); //вихід у спільну позицію, припущення: вона задана останньою у списку позицій !!!
                    //*****************
    ...
    public void input() { //вхід маркерів в переходи Петрі-об'єкта added by Inna 15.05.2016
        ...
        if (activeT.isEmpty() && isBufferEmpty()) {     //зупинка імітації за умови, що
            ...
            while (activeT.size() > 0) { // запуск переходів доки можливо та доки відсутній невчасний вхід в перехід
                // невчасні визначаються у методі findActiveT і не заносяться до списку активних
...
//        System.out.println(System.nanoTime()-t);
        setTimeInputMeasure(getTimeInputMeasure() + System.nanoTime()-t);
        setNumInput(getNumInput() + 1);
    }
     ...
     *
     */
    public void output() { //added by Inna 15.05.2016, rewritten 17.05.2016-2.06.2016
       
        if (nextObj != null) {
            listP[listP.length - 1].setExternal(true); //  can I do this during the model creation
        }

        for (PetriT transition : getListT()) { //Вихід з усіх переходів, що час виходу маркерів == поточний момент час.
            while (transition.getMinTime() == getTimeLocal() && transition.getBuffer() > 0) { //buffer>0 I think is unnecessary condition
                 long t = System.nanoTime();
                ...
                // але тільки у позиції, які не є зовнішніми
                  setTimeOutputMeasure(getTimeOutputMeasure() + System.nanoTime()-t);
                   setNumOutput(getNumOutput() + 1);
                if (nextObj != null && getOutT().contains(transition)) {
                    nextObj.addTimeExternalInput((Double) getTimeLocal()); // signal
    ...
    public void reinstateActOut(PetriT transition, PetriP place) {
        for (ArcOut arc : previousObj.getListOut()) {
            if (arc.getNumT() == transition.getNumber() && arc.getNumP() == place.getNumber()) {
                //  System.out.println("reinstate " + ", transition " + transition.getName() + ", position " + position.getName()+", "+this.getName());
                place.increaseMark(arc.getQuantity());
...
//                increaseCounter();
    ...
    public void doStatistics(double dt) {
        if (dt > 0) {
            for (PetriP place : getListPositionsForStatistica()) {
...
//    }
    ...
    /**
     * @return the timeInputMeasure
     */
    public static long getTimeInputMeasure() {
        return timeInputMeasure;
    }

    /**
     * @param aTimeInputMeasure the timeInputMeasure to set
     */
    public static void setTimeInputMeasure(long aTimeInputMeasure) {
        timeInputMeasure = aTimeInputMeasure;
    }

    /**
     * @return the timeOutputMeasure
     */
    public static long getTimeOutputMeasure() {
        return timeOutputMeasure;
    }

    /**
     * @param aTimeOutputMeasure the timeOutputMeasure to set
     */
    public static void setTimeOutputMeasure(long aTimeOutputMeasure) {
        timeOutputMeasure = aTimeOutputMeasure;
    }
    ...
    /**
     * @param aNumInput the numInput to set
     */
    public static void setNumInput(int aNumInput) {
        numInput = aNumInput;
    }
    ...
    /**
     * @param aNumOutput the numOutput to set
     */
    public static void setNumOutput(int aNumOutput) {
        numOutput = aNumOutput;
    }
}
